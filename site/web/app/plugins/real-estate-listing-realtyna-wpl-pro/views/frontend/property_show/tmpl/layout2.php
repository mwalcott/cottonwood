            ...
            <div class="wpl-large-8 wpl-medium-7 wpl-small-12 wpl-column wpl_prp_container_content_left ">
                ...
                foreach($this->wpl_properties['current']['rendered'] as $values)
                {
                    ...
                    echo '<div class="wpl_prp_show_detail_boxes">
                            ...
                            <div class="wpl-small-up-1 wpl-medium-up-1 wpl-large-up-2 wpl_prp_show_detail_boxes_cont">';
                    ...
                    foreach($values['data'] as $key => $value)
                    {
                        ...
                        {
                            echo '<div id="wpl-dbst-show'.$value['field_id'].'" class="wpl-column rows neighborhood"><label>' .__($value['name'],'wpl') .(isset($value['distance']) ? '</label> <span class="'.$value['vehicle_type'].'">'. $value['distance'] .' '. __('Minutes','wpl'). '</span>':''). '</div>';
                        }
                        elseif($value['type'] == 'feature')
                        {
                            ...
                            echo '"><label>'.__($value['name'], 'wpl').'</label>';
                        ...
                        elseif($value['type'] == 'locations' and isset($value['locations']) and is_array($value['locations']))
                        {
                            ...
                            {
                                echo '<div id="wpl-dbst-show'.$value['field_id'].'" class="wpl-column rows location" itemprop="address"><label>'.__($value['keywords'][$ii], 'wpl').' : </label>';
                        ...
                        elseif($value['type'] == 'separator')
                        {
                            ...
                            echo '<div id="wpl-dbst-show'.$value['field_id'].'" class="wpl-column rows other"><label>' .__($value['name'], 'wpl'). ' : </label><span>'. __((isset($value['value']) ? $value['value'] : ''), 'wpl') .'</span></div>';
                    }
                    ...
                    <div class="tabs_box">
                        <ul class="tabs clearfix">
                            <?php if($pshow_googlemap_activities and $this->wpl_properties['current']['raw']['show_address']): ?>
                                <li><a href="#tabs-1" data-init-googlemap="1"><?php echo __('Google Map', 'wpl') ?></a></li>
                            <?php endif; ?>
                            <?php if($pshow_bingmap_activities): ?>
                                <li><a href="#tabs-2" data-init-bingmap="1"><?php echo __('Birds Eye', 'wpl') ?></a></li>
                            <?php endif; ?>
                        </ul>
                    </div>
                    ...
                    <div class="wpl_prp_right_boxes_content">
                        <div class="wpl_prp_right_boxe_details_top clearfix">
                            <div class="wpl_prp_show_title">
                                ...
                                echo '<h2 class="location_build_up"><span itemprop="address">'. $location_string .'</span></h2>';
                                ?>
                            </div>
                            <div class="wpl_prp_listing_icon_box">
                                <?php if(trim($bedroom) != ''): ?><div class="bedroom"><?php echo $bedroom.'  <span>'.__($this->wpl_properties['current']['materials']['bedrooms']['name'], 'wpl').'</span>'; ?></div><?php endif; ?>
                                <?php if(trim($bathroom) != ''): ?><div class="bathroom"><?php echo $bathroom.'  <span>'.__($this->wpl_properties['current']['materials']['bathrooms']['name'], 'wpl').'</span>'; ?></div><?php endif; ?>
                                <?php if(trim($build_up_area) != ''): ?><div class="built_up_area"><?php echo $build_up_area; ?></div><?php endif; ?>
                            </div>
                            <?php echo '<div class="wpl_prp_right_boxe_details_bot"><div class="price_box" itemprop="price">'.$price.'</div></div>'; ?>
                            <?php if(trim($listing_id) != ''): ?><div class="wpl_prp_mls_id_box"><?php echo __($this->wpl_properties['current']['materials']['mls_id']['name'], 'wpl').'  <span itemprop="productID">#'.$listing_id.'</span>'; ?></div><?php endif; ?>
                        </div>
                        <?php
