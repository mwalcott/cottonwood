...
if($show == 'radiussearch' and !$done_this)
{
	...
	/** current values **/
	$current_value = stripslashes(wpl_request::getVar('sf_radiussearch', ''));
	...
	$html .= '<label class="wpl_search_widget_location_level_label" for="sf'.$widget_id.'_radiussearch_id">'.__('Radius', 'wpl').'<img src="'.wpl_global::get_wpl_asset_url('img/radius.png').'" alt="'.__('Radius', 'wpl').'" style="vertical-align: middle;" /></label>';
	...
	{
		$html .= '<span class="wpl_search_widget_radius_unit_label">'.__('km', 'wpl').'</span><input class="wpl_search_widget_radiussearchunit_hidden" id="sf'.$widget_id.'_radiussearchunit" name="sf'.$widget_id.'_radiussearchunit" type="hidden" value="13" />';
	}
	...
	$html .= '<input class="wpl_search_widget_location_textsearch" value="'.$current_value.'" name="sf'.$widget_id.'_radiussearch" id="sf'.$widget_id.'_radiussearch" placeholder="'.__($placeholder, 'wpl').'" />';
	...
	(function($){$(function()
    {
        $("#sf'.$widget_id.'_radiussearch").autocomplete(
        {
            search : function(){},
